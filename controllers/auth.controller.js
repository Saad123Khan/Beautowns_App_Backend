...
const loginUser = asyncHandler(async (req, res) => {
  ...
  let updatedUser = await User.findOne({email:req.body.email}).select('role email name phone isVerified');
...
const otpVerify = asyncHandler(async (req, res) => {
  ...
  const emailValid = await User.findOne({ email: req.body.email }).select('role email name phone isVerified');
  if (!emailValid) {
    return res
      .status(404)
  ...
  if (!otpFind)
   ...
   if (otpFind?.otp === req.body.otp) {
    ...
    const user =  await User.findOneAndUpdate({email :emailValid?.email}, {$set :  { isVerified : true }}, { new: true }).select('role email name phone isVerified');
...
export { createUser,loginUser, otpVerify, logout ,forgetPassword,updatePassword};
