...
const createStoreCategory = asyncHandler(async (req, res) => {
  ...
  if (category) {
    ...
    const storeCategories = await StoreCategories.find({
      ...(req.query.role === "store" && { store_Id: category?.store_Id }),
      isDeleted: false,
    });
    ...
    return res.status(201).send({
      status: true,
      message: "Sucessfully created service category",
      category,
...
const getAllCategories = asyncHandler(async (req, res) => {
  const serviceCategories = await StoreCategories.find({
    ...(req.query.role === "store" && { store_Id: req.params.id }),
    isDeleted: false,
  }).populate("store_Id");
  if (serviceCategories?.length > 0) {
    return res.status(200).send({ status: true, serviceCategories });
  } else {
...
const updateCategory = asyncHandler(async (req, res) => {
  let storeCategories = await StoreCategories.findOne({
    _id: req.params.id,
    isDeleted: false,
  });
  ...
  if (storeCategories) {
    ...
    return res.status(200).send({
      status: true,
      message: "Updated successfully service category",
      serviceCategories: findCategory,
    });
  } else {
    return res
      .status(404)
...
const delete_catgory = asyncHandler(async (req, res) => {
  ...
  if (delete_cat) {
    ...
    let services = [];
    const storeCategories = await StoreCategories.find({
      ...(req.query.role === "store" && { store_Id: delete_cat?.store_Id }),
